...
func _version_conversion():
	var EditorGlobals = load("res://project/addons/gut/gui/editor_globals.gd")
	EditorGlobals.create_temp_directory()

	var VersionConversion = load("res://project/addons/gut/version_conversion.gd")
	VersionConversion.convert()

func _enter_tree():
	_version_conversion()

	_bottom_panel = preload('res://project/addons/gut/gui/GutBottomPanel.tscn').instantiate()
